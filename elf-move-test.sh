...
function test_run() {
    ...
    local oddothdir="${bdir}/usr/foo/usr/lib64"
    ...
    local ooddothdir="${odir}/usr/foo/usr/lib64"
    ...
    local ooddhash="${odir}/usr/share/clear/optimized-elf/other718918195af7bf5d13dc6b59b096400fe6ef6b6d4bce9da60489b141158a8eaf"
    if [ $avx -eq 2 ]; then
        ...
        local oddhash="${odir}/usr/share/clear/optimized-elf/other11636a1f678ba08330ba0491cb9d3eba062d1318f00a01d0af7d6b950a281c54"
    else
        ...
        local oddhash="${odir}/usr/share/clear/optimized-elf/otherca686cd656124dd9f6b8a29530044a2a30ce397479af4d81f54a660405eecac4"
    fi
    ...
    echo -n -e \\x7f\\x45\\x4c\\x46\\x00\\x00\\x00\\x00\\x00\\x00 > "${ooddothdir}/oofile"
